...
import { StructuredCaseForm } from '@/components/ui/StructuredCaseForm';
import { CaseView } from '@/components/ui/CaseView';
...
import type { LLErrorCategory } from '@/lib/cases/errors';
...
export default function StudyChat({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleStructuredCaseSubmit = async (input: { topic: string; learningObjective: string; pedagogicalRationale: string; sourceText: string }) => {
    setTopicSelected(true);
    setGenerationError(null);

    if (resolvedId !== 'new' && chatTitle === 'Nova Sessão de Estudo') {
      handleTitleChange(`Estudo: ${input.topic}`);
    }

    const initMsg: StudyMessage = {
      id: Date.now().toString(),
      role: 'model',
      content: `Gerando caso clínico estruturado sobre: **${input.topic}**. Aguarde...`,
    };
    setMessages([initMsg]);
    setLoading(true);
    ...
    try {
      ...
      if (!response.ok) {
        let errorMessage = 'Ocorreu um erro ao gerar o caso. Tente novamente.';
        try {
          const errBody = await response.json();
          if (errBody?.error) {
            errorMessage = errBody.error;
          } else if (errBody?.category) {
            errorMessage = getUserFacingErrorMessage({ category: errBody.category as LLErrorCategory });
          }
        } catch {
        ...
        if (response.status === 422 || response.status === 400) {
          ...
          if (errBody?.validationErrors) {
            const error = errBody.validationErrors as CaseGenerationInputError[];
            const first = error[0];
            errorMessage = first ? `${first.field}: ${first.message}` : errorMessage;
          }
        }
      ...
      const result = await response.json();
      const caseMsg: StudyMessage = {
        id: Date.now().toString(),
        role: 'model',
        content: 'Case clínico gerado.',
        caseData: result.case as ClinicalCase,
      };

      setMessages([caseMsg]);
      await saveMessageToDB('user', `Quero estudar sobre: ${input.topic} com objetivo: ${input.learningObjective}`);
      await saveMessageToDB('model', `Caso estruturado gerado para: ${input.topic}`, {
        caseData: result.case,
      });
    } catch (e) {
      console.error(e);
      setGenerationError('Ocorreu um erro ao gerar o caso. Tente novamente.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <main 
          ...
          {!topicSelected ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-lg)', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
              <h2 style={{ fontSize: '1.25rem', fontWeight: 600 }}>Criar Sessão de Estudo</h2>
              <p style={{ color: 'var(--color-semantic-text-textlight)', textAlign: 'center', maxWidth: '400px' }}>
                Escolha uma especialidade, digite um tema, ou preencha os campos estruturados para gerar um caso clínico fundamentado.
              </p>
              ...
              <div style={{ width: '100%', maxWidth: '500px', marginTop: 'var(--spacing-md)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-ml)', marginBottom: 'var(--spacing-md)' }}>
                  <div style={{ flex: 1, height: '1px', backgroundColor: 'var(--color-semantic-text-textlight)', opacity: 0.4 }} />
                  <span style={{ fontSize: '0.85rem', color: 'var(--color-semantic-text-textlight)' }}>ou geração estruturada</span>
                  <div style={{ flex: 1, height: '1px', backgroundColor: 'var(--color-semantic-text-textlight)', opacity: 0.4 }} />
                </div>
                ...
                {generationError && (
                  <div style={{
                    ...
                    backgroundColor: 'rgba(214, 93, 93, 0.12)',
                    borderRadius: '16px',
                    color: '#b03030',
                    fontSize: '0.9rem',
                ...
                <StructuredCaseForm onSubmit={handleStructuredCaseSubmit} disabled={loading} />
              </div>
            </div>
          ) : (
            <>
              {messages.map((msg) => (
                <div key={msg.id} style={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
                  <div style={{ 
                    ...
                    {msg.caseData ? (
                      <div style={{ width: '100%' }}>
                        <CaseView caseData={msg.caseData} />
                      </div>
                    ) : msg.isMCQ ? (
